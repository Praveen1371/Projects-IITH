...
Re = 1000;
dt = 0.001; maxIt = 1e5; maxe = 1e-7; % Time Step; Max iter; Max error
%%% SETUP 1D GRID
...
(Wall_Velocity*L/Re)*(Vop(ip,j)+Vop(im,j)-4*Vop(i,j)+Vop(i,jp)+Vop(i,jm))/(h^2))*dt;
...
[X,Y] = meshgrid(x,y);
figure(2); plot(x,v(:,round(Nx/2)));
title('Centerline y-direction velocity');
xlabel('x/L'); ylabel('v/U'); axis('square'); xlim([0 L]); grid on
N = 1000; xstart = max(x)*rand(N,1); ystart = max(y)*rand(N,1);
[X,Y] = meshgrid(x,y);
